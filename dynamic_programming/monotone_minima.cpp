 ...
 */
void dfs(ll il, ll ir, ll jl, ll jr, vector<ll> &ans, function<ll(ll, ll)> &f) {
  if(il >= ir) return;
  ...
  dfs(il, im, jl, jm, ans, f);
  dfs(im + 1, ir, jm, jr, ans, f);
}
